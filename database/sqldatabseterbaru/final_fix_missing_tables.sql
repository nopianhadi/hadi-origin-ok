-- ============================================
-- FINAL FIX FOR MISSING TABLES
-- ============================================
-- This creates the missing tables and populates them with data
-- Handles duplicates and avoids subquery issues
-- ============================================
...
INSERT INTO public.blog_posts (
  title, slug, excerpt, content, image, category, tags, author, read_time, is_published, is_featured, meta_title, meta_description
) VALUES
(
  'Building Modern Web Applications with React and Next.js',
  'building-modern-web-applications-react-nextjs',
...
(
  ...
  'mobile-app-development-react-native-guide',
...
-- ============================================
-- TECHNOLOGY CATEGORIES DATA (Insert first)
-- ============================================
...
-- ============================================
-- TECHNOLOGIES DATA (Insert after categories, using specific IDs)
-- ============================================

-- Get category IDs and insert technologies one by one to avoid subquery issues
DO $$
DECLARE
...
BEGIN
    -- Get category IDs
    SELECT id INTO frontend_cat_id FROM public.technology_categories WHERE title_en = 'Frontend Development' LIMIT 1;
    SELECT id INTO mobile_cat_id FROM public.technology_categories WHERE title_en = 'Mobile Development' LIMIT 1;
    SELECT id INTO backend_cat_id FROM public.technology_categories WHERE title_en = 'Backend & Database' LIMIT 1;
    SELECT id INTO cloud_cat_id FROM public.technology_categories WHERE title_en = 'Cloud & DevOps' LIMIT 1;
    
    -- Insert Frontend technologies
    IF frontend_cat_id IS NOT NULL THEN
        INSERT INTO public.technologies (category_id, name, level, color, sort_order) VALUES
        (frontend_cat_id, 'React', 'Expert', 'bg-blue-500', 1),
        (frontend_cat_id, 'Next.js', 'Expert', 'bg-gray-800', 2),
        (frontend_cat_id, 'TypeScript', 'Advanced', 'bg-blue-600', 3),
        (frontend_cat_id, 'Tailwind CSS', 'Expert', 'bg-cyan-500', 4),
        (frontend_cat_id, 'Vue.js', 'Advanced', 'bg-green-500', 5),
        (frontend_cat_id, 'Vite', 'Advanced', 'bg-purple-500', 6)
        ON CONFLICT DO NOTHING;
    END IF;
    
    -- Insert Mobile technologies
    IF mobile_cat_id IS NOT NULL THEN
        INSERT INTO public.technologies (category_id, name, level, color, sort_order) VALUES
        (mobile_cat_id, 'React Native', 'Expert', 'bg-blue-500', 1),
        (mobile_cat_id, 'Expo', 'Advanced', 'bg-gray-800', 2),
        (mobile_cat_id, 'Flutter', 'Intermediate', 'bg-blue-400', 3),
        (mobile_cat_id, 'PWA', 'Advanced', 'bg-green-500', 4)
        ON CONFLICT DO NOTHING;
    END IF;
    
    -- Insert Backend technologies
    IF backend_cat_id IS NOT NULL THEN
        INSERT INTO public.technologies (category_id, name, level, color, sort_order) VALUES
        (backend_cat_id, 'Node.js', 'Expert', 'bg-green-600', 1),
        (backend_cat_id, 'Python', 'Advanced', 'bg-yellow-500', 2),
        (backend_cat_id, 'PostgreSQL', 'Expert', 'bg-blue-700', 3),
        (backend_cat_id, 'Supabase', 'Expert', 'bg-green-500', 4),
        (backend_cat_id, 'MongoDB', 'Advanced', 'bg-green-700', 5)
        ON CONFLICT DO NOTHING;
    END IF;
    
    -- Insert Cloud technologies
    IF cloud_cat_id IS NOT NULL THEN
        INSERT INTO public.technologies (category_id, name, level, color, sort_order) VALUES
        (cloud_cat_id, 'AWS', 'Advanced', 'bg-orange-500', 1),
        (cloud_cat_id, 'Vercel', 'Expert', 'bg-gray-800', 2),
        (cloud_cat_id, 'Docker', 'Advanced', 'bg-blue-600', 3),
        (cloud_cat_id, 'GitHub Actions', 'Advanced', 'bg-gray-700', 4)
        ON CONFLICT DO NOTHING;
    END IF;
END $$;
...
-- SETUP COMPLETE
-- ============================================
-- All missing tables created and populated!
