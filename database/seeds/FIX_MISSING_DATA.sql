-- ============================================
-- FIX MISSING DATA - Based on Test Results
-- ============================================
-- Date: 2025-11-02
-- Purpose: Add missing data found during comprehensive testing
-- Issues Fixed:
...
-- First, get or create Backend & Database category
DO $$
DECLARE
...
BEGIN
    ...
    IF backend_category_id IS NULL THEN
        INSERT INTO technology_categories (name_en, name_id, icon, display_order, is_active)
        VALUES (
            'Backend & Database',
            'Backend & Database',
            'database',
    ...
    INSERT INTO technologies (category_id, name, icon, proficiency, description_en, description_id, is_active, display_order)
    VALUES
        -- Node.js
...
BEGIN
    ...
    WHERE tc.name_en = 'Backend & Database';
...
SELECT 
    ...
    (SELECT COUNT(*) FROM technologies WHERE category_id IN 
        (SELECT id FROM technology_categories WHERE name_en = 'Backend & Database')) as backend_technologies,
